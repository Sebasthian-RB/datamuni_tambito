    ...
    <form action="{{ route('committees.update', $committee->id) }}" method="POST">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Formulario para editar comité</h3>
            </div>
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('urban_core') is-invalid @enderror" id="urban_core" name="urban_core" required>
                        @foreach ($urbanCores as $core)
                            <option value="{{ $core }}" {{ old('urban_core', $committee->urban_core) == $core ? 'selected' : '' }}>
                                {{ $core }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control @error('beneficiaries_count') is-invalid @enderror" id="beneficiaries_count" name="beneficiaries_count" value="{{ old('beneficiaries_count', $committee->beneficiaries_count) }}" required>
                    ...
                    <select name="sector_id" id="sector_id" class="form-control" required>
                        @foreach ($sectors as $sector)
                            <option value="{{ $sector->id }}" 
                                @if(old('sector_id', $committee->sector_id) == $sector->id) selected @endif>
                                {{ $sector->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="card-footer">
                <button type="submit" class="btn btn-success">Actualizar Comité</button>
