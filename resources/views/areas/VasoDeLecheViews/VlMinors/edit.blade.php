...
@section('title', 'Editar VlMinor')

@section('content_header')
    <h1>Editar VlMinor</h1>
@stop
    ...
    <form action="{{ route('vl_minors.update', $vlMinor->id) }}" method="POST">
        ...
        <div class="card">
            <div class="card-header" style="background-color: #3B1E54; color: #FFFFFF;">
                <h3 class="card-title">Formulario para editar vlMinor</h3>
            </div>
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="date" class="form-control @error('birth_date') is-invalid @enderror" id="birth_date" name="birth_date" value="{{ old('birth_date', $vlMinor->birth_date ? $vlMinor->birth_date->format('Y-m-d') : '') }}" required>
                    @error('birth_date')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('sex_type') is-invalid @enderror" id="sex_type" name="sex_type" required>
                        ...
                        @foreach($sexTypes as $key => $value)
                            <option value="{{ $key }}" {{ old('sex_type', $vlMinor->sex_type) == $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
                    @error('sex_type')
                        <span class="invalid-feedback">{{ $message }}</span>
                ...
                <div class="form-group">
                    ...
                    <input type="date" class="form-control @error('withdrawal_date') is-invalid @enderror" 
                        id="withdrawal_date" name="withdrawal_date" 
                    ...
                    <div class="row">
                        ...
                        <select name="kinship" id="kinship" class="form-control @error('kinship') is-invalid @enderror" required>
                            ...
                            @foreach($kinships as $kinship)
                                <option value="{{ $kinship }}" {{ old('kinship', $vlMinor->kinship) == $kinship ? 'selected' : '' }}>
                                    {{ $kinship }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="card-footer">
                <button type="submit" class="btn btn-success" style="background-color: #9B7EBD; color: white; border: #9B7EBD;">Guardar Cambios</button>
