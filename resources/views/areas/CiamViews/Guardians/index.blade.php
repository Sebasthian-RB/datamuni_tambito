                        ...
                        <tbody>
                            @foreach ($guardians as $guardian)
                                <tr>
                                    ...
                                    <td>{{ $guardian->given_name }} {{ $guardian->paternal_last_name }}
                                        {{ $guardian->maternal_last_name }}</td>
                                    <td>
                                    ...
                                    <td>
                                        @can('ver detalles')
                                            <a href="{{ route('guardians.show', $guardian->id) }}" class="btn btn-sm"
                                                style="background-color: #6E8E59; color: white;" aria-label="Ver detalles">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                        @endcan
                                        @can('editar')
                                            <a href="{{ route('guardians.edit', $guardian->id) }}" class="btn btn-sm"
                                                style="background-color: #CAE0BC; color: black;" aria-label="Editar guardián">
                                        ...
                                        <form action="{{ route('guardians.destroy', $guardian->id) }}" method="POST"
                                            ...
                                            @can('eliminar')
                                                <button type="button" class="btn btn-sm"
                                                    style="background-color: #780C28; color: white;" data-bs-toggle="modal"
                                                    data-bs-target="#deleteGuardianModal{{ $guardian->id }}"
                                                    aria-label="Eliminar guardián">
                                ...
                                <div class="modal fade" id="deleteGuardianModal{{ $guardian->id }}" tabindex="-1"
                                    aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                ¿Estás seguro de que deseas eliminar al guardián
                                                <strong>{{ $guardian->given_name }}
                                                    {{ $guardian->paternal_last_name }}</strong>?

                                                @if ($guardian->elderlyAdults->isNotEmpty())
                                                    <div class="alert alert-warning mt-3">
                                                        <strong>Advertencia:</strong> Los siguientes adultos mayores
                                                        quedarán sin guardián:
                                                        <ul>
                                                            @foreach ($guardian->elderlyAdults as $elderly)
                                                                <li>{{ $elderly->given_name }}
                                                                    {{ $elderly->paternal_last_name }}</li>
                                                            @endforeach
                                                        </ul>
                                                    </div>
                                                @endif
