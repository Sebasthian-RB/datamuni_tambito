...
@section('content')
            ...
            <form action="{{ route('guardians.update', $guardian->id) }}" method="POST" autocomplete="off">
                ...
                <div class="form-group">
                    ...
                    <select id="document_type" name="document_type"
                        ...
                        @foreach (['DNI', 'Pasaporte', 'Carnet', 'Cedula'] as $type)
                            <option value="{{ $type }}"
                                {{ old('document_type', $guardian->document_type) == $type ? 'selected' : '' }}>
                                {{ $type }}</option>
                        @endforeach
                    </select>
                    @error('document_type')
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="id" name="id"
                        class="form-control @error('id') is-invalid @enderror" value="{{ old('id', $guardian->id) }}"
                        placeholder="Seleccione tipo de documento primero" required>
                    @error('id')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    <label for="paternal_last_name" class="font-weight-bold" style="color: #6E8E59;">Apellido
                        Paterno</label>
                    <input type="text" id="paternal_last_name" name="paternal_last_name"
                        ...
                        value="{{ old('paternal_last_name', $guardian->paternal_last_name) }}"
                ...
                <div class="form-group">
                    <label for="maternal_last_name" class="font-weight-bold" style="color: #6E8E59;">Apellido
                        Materno</label>
                    <input type="text" id="maternal_last_name" name="maternal_last_name"
                        ...
                        value="{{ old('maternal_last_name', $guardian->maternal_last_name) }}"
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="phone_number" name="phone_number"
                        ...
                        placeholder="Ingrese 9 dígitos numéricos" maxlength="9" pattern="[0-9]{9}"
                        title="Debe ingresar exactamente 9 dígitos numéricos">
                    @error('phone_number')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                    <small class="form-text text-muted">Ingrese solo números (9 dígitos)</small>
                </div>

                <!-- Campo para Relación con el Adulto Mayor -->
                <div class="form-group">
                    <label for="relationship" class="font-weight-bold" style="color: #6E8E59;">Relación con el Adulto
                        Mayor</label>
                    <select id="relationship" name="relationship"
                        ...
                        @foreach (['Hijo/a', 'Esposo/a', 'Nieto/a', 'Hermano/a', 'Cuidador externo', 'Otro'] as $relation)
                            <option value="{{ $relation }}"
                                {{ old('relationship', $guardian->relationship) == $relation ? 'selected' : '' }}>
                                {{ $relation }}</option>
                        @endforeach
                    </select>
                    @error('relationship')
                ...
                <div class="form-group {{ old('relationship', $guardian->relationship) === 'Otro' ? '' : 'd-none' }}"
                    id="other_relationship_div">
                    ...
                    <input type="text" id="other_relationship" name="other_relationship" class="form-control"
                        value="{{ old('other_relationship', $guardian->other_relationship ?? '') }}"
                ...
                <div class="text-center">
                    <button type="submit" class="btn"
                        style="background-color: #6E8E59; color: white;">Actualizar</button>
                    <a href="{{ route('guardians.index') }}" class="btn btn-secondary"
                        style="background-color: #780C28; color: white;">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Limpiar y formatear el valor actual si existe
            if (phoneInput.value) {
                phoneInput.value = phoneInput.value.replace(/[^0-9]/g, '').substring(0, 9);
            }
        });
    </script>
@stop
