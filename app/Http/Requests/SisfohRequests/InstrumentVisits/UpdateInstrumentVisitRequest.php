...
/**
 * Form Request para actualizar un instrumento/visita.
 * Aquí se definen las reglas de validación para la actualización de instrumentos/visitas.
 */

class UpdateInstrumentVisitRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        return [
            ...
            'descriptions' => [
                'nullable',
                'string',
                'max:1000', // Descripciones con límite de 1000 caracteres
            ],
            'instrument_id' => [
                'required',
                'integer',
                'exists:instruments,id', // El ID del instrumento debe existir en la tabla instruments
                Rule::unique('instrument_visits')->ignore($this->route('instrument_visit')), // Ignora la validación de unicidad para el registro actual al actualizarlo
            ],
            'visit_id' => [
                'required',
                'integer',
                'exists:visits,id', // El ID de la visita debe existir en la tabla visits
                Rule::unique('instrument_visits')->ignore($this->route('instrument_visit')), // Ignora la validación de unicidad para el registro actual al actualizarlo
            ],
